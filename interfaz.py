...
class VentanaPrincipal(QMainWindow):
    ...
    def cargar_datos_en_tablero(self):
        self.cargando_datos = True # 🔴 SEMÁFORO ROJO (Bloquear lógica)
        for lista in self.listas_bases.values():
            lista.clear()

            # ESTILO TAXI AMARILLO
            lista.setStyleSheet("""
                ...
                QListWidget::item {
                    ...
                    border-radius: 10px; /* Bordes un poco menos redondos para ahorrar espacio */
                    margin: 2px;
                }
                QListWidget::item:hover { background-color: #D4A017; border: 2px solid #FFFFFF; }
                QListWidget::item:selected { background-color: #FFD700; color: black; }
            """)
            lista.setViewMode(QListWidget.ViewMode.IconMode)
            lista.setResizeMode(QListWidget.ResizeMode.Adjust)
            lista.setSpacing(6) # Menos espacio entre fichas
            lista.setDragEnabled(True); lista.setAcceptDrops(True); lista.setDefaultDropAction(Qt.DropAction.MoveAction)

        taxis = self.db.obtener_taxis_activos()
        for taxi in taxis:
            bid = taxi['base_actual_id']
            if bid in self.listas_bases:
                item = TaxiItem(str(taxi['numero_economico']))
                
                # --- TAMAÑO REDUCIDO ---
                f = QFont(); f.setPointSize(14); f.setBold(True) # Fuente 14
                item.setFont(f)
                item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                item.setSizeHint(QSize(65, 65)) # Ficha 65x65 (Más pequeña)
                
                item.setData(Qt.ItemDataRole.UserRole, taxi['id'])
                self.listas_bases[bid].addItem(item)

        self.cargando_datos = False # 🟢 SEMÁFORO VERDE (Ya puedes detectar movimientos)
    ...
    def _ejecutar_actualizacion_bd(self, lista_destino, indice_item):
        ...
        if id_base_nueva and taxi_id_bd:
            ...
            res = cur.fetchone()
            conn.close()
            id_ant = res['base_actual_id'] if res else None
            
            # === LÓGICA DE REBOTE ===
            if id_ant == id_base_nueva:
                # Si es base inactiva (Taller, etc), ignorar
                if id_base_nueva in [12, 90, 91]:
                    lista_destino.sortItems(Qt.SortOrder.AscendingOrder)
                    return
                # Si es Local/Foráneo, CONTINUAMOS para permitir Ligar Viaje
            
            # --- PROCESO ---
            # CASO A: VIAJES (Local 93, Foráneo 92)
            if id_base_nueva in [92, 93]:
                if id_ant in [92, 93]: # Viene de otro viaje -> PREGUNTAR
                    ans = QMessageBox.question(self, "Ligar Viaje", f"El taxi {taxi_num} ya tiene viaje activo.\n¿Finalizar anterior e iniciar uno nuevo?", QMessageBox.StandardButton.Yes|QMessageBox.StandardButton.No)
                    ...
                    # Viaje Nuevo Normal
                    if id_ant in [12, 90, 91]: self.db.hora_entrada(taxi_id_bd)
                    self.abrir_ventana_nuevo_viaje(taxi_id_bd, taxi_num, id_ant)

            # CASO B: INACTIVOS
            elif id_base_nueva in [12, 90, 91]:
                if id_ant not in [12, 90, 91]:
                    self.db.hora_salida(taxi_id_bd)
                    self.db.registrar_fin_viaje(taxi_id_bd)

            # CASO C: BASES NORMALES
    ...
    def filtrar_taxis_tablero(self, texto):
        ...
        for lista in self.listas_bases.values():
            for i in range(lista.count()):
                it = lista.item(i)
                it.setHidden(busq and it.text().lower() != busq)

    def filtrar_tabla_historial(self, texto):
        busq = texto.lower().strip()
        for r in range(self.tabla_reportes.rowCount()):
            it = self.tabla_reportes.item(r, 3)
            if it: self.tabla_reportes.setRowHidden(r, busq and it.text().lower() != busq)
    ...
    def exportar_pdf_unidad(self):
        ...
        if not numero:
            QMessageBox.warning(self, "Atención", "Escribe un número de taxi.")
            return
            
        taxi_id = self.db.obtener_id_por_numero(numero)
        if not taxi_id: 
            QMessageBox.warning(self, "Error", "El taxi no existe.")
            return
        ...
        if not detalle_viajes:
            QMessageBox.information(self, "Vacio", f"Sin viajes para el taxi {numero}.")
            return
        ...
        texto_fecha = ""
        if periodo in ["DIA", "HOY"]: texto_fecha = f"{dia} de {mes} de {anio}"
        elif periodo == "MES": texto_fecha = f"{mes} {anio}"
        elif periodo == "AÑO": texto_fecha = f"Año {anio}"
        else: texto_fecha = "Reporte Histórico"
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", f"Fallo al crear PDF:\n{e}")
    ...
    def construir_pagina_reportes_globales(self, parent):
        ...
        # Cuadro más grande
        f = QFrame(); f.setFixedSize(600, 480); f.setStyleSheet("background-color: #1E293B; border-radius: 20px; border: 2px solid #334155;")
        fl = QVBoxLayout(f); fl.setSpacing(25); fl.setContentsMargins(40,40,40,40)
        
        lbl = QLabel("GENERADOR DE REPORTES"); lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lbl.setStyleSheet("color: #FACC15; font-size: 28px; font-weight: bold; border: none;")
        
        h = QHBoxLayout()
        # Campos de texto y fecha grandes
        self.date_global = QDateEdit(); self.date_global.setCalendarPopup(True); self.date_global.setDate(QDate.currentDate())
        self.date_global.setStyleSheet("background-color: #0F172A; color: white; padding: 12px; font-size: 16px; border-radius: 5px;")
        
        self.cmb_periodo_global = QComboBox(); self.cmb_periodo_global.addItems(["DIA","MES","AÑO","SIEMPRE"])
        self.cmb_periodo_global.setStyleSheet("background-color: #0F172A; color: white; padding: 12px; font-size: 16px; border-radius: 5px;")
        ...
        # Botón grande
        b = QPushButton("📄 DESCARGAR REPORTE GLOBAL")
        b.setStyleSheet("background-color: #00D1FF; color: #0F172A; font-weight: bold; font-size: 18px; padding: 20px; border-radius: 10px;")
        b.clicked.connect(self.generar_pdf_corporativo)
        
        fl.addWidget(lbl); fl.addLayout(h); fl.addStretch(); fl.addWidget(b)
...
if __name__ == "__main__":
    ...
    # 1. Anti-doble instancia
    mem = QSharedMemory("SistemaTaxisZorro_Unique_Key_v1")
    if not mem.create(1):
        QMessageBox.warning(None, "Sistema", "El sistema ya se está ejecutando.")
        sys.exit(0)

    # 2. VERIFICACIÓN Y CREACIÓN DE BD (DELEGADA)
    nombre_db = "taxis.db"
    if not os.path.exists(nombre_db):
        resp = QMessageBox.question(None, "Bienvenido", 
            "No se detectó base de datos.\n¿Deseas crear el sistema nuevo V2.0 (con multas, bitácora y bases especiales)?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        
        if resp == QMessageBox.StandardButton.Yes:
            # ¡AQUÍ LLAMAMOS A LA CLASE GESTOR!
            if GestorBaseDatos.crear_nueva_bd_v2(nombre_db):
                QMessageBox.information(None, "Éxito", "Sistema V2.0 creado correctamente.\nUsuario Admin: 'admin_pass' -> '1234'")
            else:
                QMessageBox.critical(None, "Error", "No se pudo crear el archivo de base de datos.")
                sys.exit(0)
        else:
            sys.exit(0)
    else:
        # Si existe, aseguramos que sea visible por si acaso
        try: ctypes.windll.kernel32.SetFileAttributesW(nombre_db, 0x80)
        except: pass
